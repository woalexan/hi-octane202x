...
RegionMode::~RegionMode() {
    ...
    if (mGuiRegionMode.RemoveButton != nullptr) {
        mGuiRegionMode.RemoveButton->remove();
    }

    if (mGuiRegionMode.AddButton != nullptr) {
        mGuiRegionMode.AddButton->remove();
    }
...
void RegionMode::AddRegionTableEntry(irr::u8 entryNr, MapTileRegionStruct* whichRegion) {
    ...
    //if for last entry add "New Region" line
    if (whichRegion == nullptr) {
       irr::core::stringw entryNrStr(entryNr);
       typeStr.append(L"New Region");
...
void RegionMode::UpdateRegionTable() {
    ...
    irr::u8 currNrRegions = (irr::u8)(mParentSession->mLevelRes->mMapRegionVec->size());

    mGuiRegionMode.RegionTable->clearRows();

    //add all possible regions into the region table
    for (irr::u8 entryIdx = 0; entryIdx < currNrRegions; entryIdx++) {
...
void RegionMode::UpdateUiDialog() {
    ...
    if (mLastSelectedRegion == nullptr) {
        ...
        mGuiRegionMode.RemoveButton->setEnabled(false);
        mGuiRegionMode.RemoveButton->setVisible(false);

        mGuiRegionMode.AddButton->setEnabled(true);
        mGuiRegionMode.AddButton->setVisible(true);
    } else {
        mGuiRegionMode.ApplyButton->setEnabled(true);
        mGuiRegionMode.ApplyButton->setVisible(true);

        mGuiRegionMode.RemoveButton->setEnabled(true);
        mGuiRegionMode.RemoveButton->setVisible(true);
...
void RegionMode::CreateWindow() {
    irr::core::dimension2d<irr::u32> dim ( 400, 300 );
    ...
    mGuiRegionMode.RegionTable = mParentSession->mParentEditor->mGuienv->addTable( rect<s32>( 10, 30, 385, dim.Height - 140), Window, GUI_ID_REGIONMODEWINDOW_REGIONTABLE);
    ...
    mGuiRegionMode.RegionTypeComboBoxLabel = mParentSession->mParentEditor->mGuienv->addStaticText ( L"Region Type:",
                                      rect<s32>( 10, dim.Height - 135, 160, dim.Height - 110 ),false, false, Window, -1, false );

    mGuiRegionMode.RegionTypeComboBox = mParentSession->mParentEditor->mGuienv->addComboBox(rect<s32>( 10, dim.Height - 115, 100, dim.Height - 90),
    ...
    mGuiRegionMode.AddButton = mParentSession->mParentEditor->mGuienv->addButton(core::recti(mx, my + 35, mx + 50, my + 60), Window, GUI_ID_REGIONMODEWINDOW_ADDBUTTON, L"Add");
    mGuiRegionMode.RemoveButton = mParentSession->mParentEditor->mGuienv->addButton(core::recti(mx, my + 75, mx + 50, my + 100), Window, GUI_ID_REGIONMODEWINDOW_REMOVEBUTTON, L"Remove");
...
bool RegionMode::PlausiCheckCoordinates(irr::f32 XMin, irr::f32 YMin, irr::f32 XMax, irr::f32 YMax) {
    ...
    for (it = mParentSession->mLevelRes->mMapRegionVec->begin(); it != mParentSession->mLevelRes->mMapRegionVec->end(); ++it) {
       if ((mLastSelectedRegion != nullptr) && (mLastSelectedRegion->regionId != (*it)->regionId)) {
...
void RegionMode::AddRegion() {
    irr::u32 currentCheckBoxSelRegion;

    //currentComboBoxSelIdx == -1 means that no ComboBox entry is currently selected
    ...
    if (mLastSelectedRegion == nullptr) {
        if (!mParentSession->mLevelRes->AddRegion(coord1, coord2, currentCheckBoxSelRegion)) {
...
void RegionMode::OnButtonClicked(irr::s32 buttonGuiId) {
    switch (buttonGuiId) {
        ...
        case GUI_ID_REGIONMODEWINDOW_ADDBUTTON: {
             AddRegion();
             break;
        }

        case GUI_ID_REGIONMODEWINDOW_REMOVEBUTTON: {
             RemoveRegion();
             break;
        }
