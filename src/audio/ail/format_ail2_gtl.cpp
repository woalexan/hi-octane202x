...
bool AIL_GTL::detect(const std::string &filePath, char *)
{
...
FfmtErrCode AIL_GTL::loadFile(std::string filePath, FmBank &bank)
{
    ...
    for(uint32_t i = 0; i < totalInsts; i++)
    {
        ...
        file.seekg(h.offset, std::ios::beg);
        ...
        else
        {
            ...
            currFPos = file.tellg();
            file.seekg(currFPos + (insLen - 24), std::ios::beg);
        }
...
FfmtErrCode AIL_GTL::saveFile(std::string filePath, FmBank &bank)
{
...
//            //OP4
         ...
         file.write(char_p(odata), ins_len);
         afterWr = file.tellp();
...
std::string AIL_GTL::formatName() const
{
    return "Audio Interface Library (Miles) bank";
}

std::string AIL_GTL::formatExtensionMask() const
{
    return "*.opl *.ad";
}

std::string AIL_GTL::formatDefaultExtension() const
{
    return "opl";
}
