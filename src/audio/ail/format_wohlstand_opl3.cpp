...
bool WohlstandOPL3::detect(const std::string &, char *magic)
{
    return (strncmp(magic, wopl3_magic, 11) == 0);
}

bool WohlstandOPL3::detectInst(const std::string &, char *magic)
{
    return (strncmp(magic, wopli_magic, 11) == 0);
}

static bool readInstrument(std::ifstream &file, FmBank::Instrument &ins, uint16_t &version, bool hasSoundKoefficients = true)
{
    ...
    if(version >= 3)
    {
        file.read(char_p(idata), WOPL_INST_SIZE_V3);
        got = file.gcount();
    ...
    {
        file.read(char_p(idata), WOPL_INST_SIZE_V2);
        readBytes = file.gcount();

        if(readBytes != WOPL_INST_SIZE_V2)
            return false;
    }
...
static bool writeInstrument(std::ofstream &file, FmBank::Instrument &ins, bool hasSoundKoefficients = true)
{
    size_t beforeWr;
    size_t afterWr;
    size_t bytesWr;
    ...
    if(hasSoundKoefficients)
    {
        ...
        beforeWr = file.tellp(); //current pos
        file.write(char_p(odata), WOPL_INST_SIZE_V3);
        afterWr = file.tellp();
        bytesWr = afterWr - beforeWr;

        if (bytesWr != WOPL_INST_SIZE_V3)
            return false;
        ...
        beforeWr = file.tellp(); //current pos
        file.write(char_p(odata), WOPL_INST_SIZE_V2);
        afterWr = file.tellp();
        bytesWr = afterWr - beforeWr;

        if (bytesWr != WOPL_INST_SIZE_V2)
            return false;
...
FfmtErrCode WohlstandOPL3::loadFile(std::string filePath, FmBank &bank)
{
    int err = 0;
    ...
    fileData = new std::vector<uint8_t>();
...
FfmtErrCode WohlstandOPL3::saveFile(std::string filePath, FmBank &bank)
{
    ...
    for (it = outFile.begin(); it != outFile.end(); ++it) {
        beforeWr = file.tellp(); //current pos
        file.write(static_cast<char*>(&*it), 1);
        afterWr = file.tellp();
        bytesWr = afterWr - beforeWr;
...
std::string WohlstandOPL3::formatName() const
{
    return "Standard OPL3 bank by Wohlstand";
}

std::string WohlstandOPL3::formatExtensionMask() const
{
    return "*.wopl";
}

std::string WohlstandOPL3::formatDefaultExtension() const
{
    return "wopl";
}
...
FfmtErrCode WohlstandOPL3::loadFileInst(std::string filePath, FmBank::Instrument &inst, bool *isDrum)
{
    char magic[32];
    memset(magic, 0, 32);
    uint16_t version = 0;
...
FfmtErrCode WohlstandOPL3::saveFileInst(std::string filePath, FmBank::Instrument &inst, bool isDrum)
{
    ...
    beforeWr = file.tellp(); //current pos
    file.write(char_p(wopli_magic), 11);
    afterWr = file.tellp();
    bytesWr = afterWr - beforeWr;
    ...
    beforeWr = file.tellp(); //current pos
    file.write(char_p(&isDrumFlag), 1);
    afterWr = file.tellp();
    bytesWr = afterWr - beforeWr;
...
std::string WohlstandOPL3::formatInstName() const
{
    return "Standard OPL3 instrument by Wohlstand";
}

std::string WohlstandOPL3::formatInstExtensionMask() const
{
    return "*.opli";
}

std::string WohlstandOPL3::formatInstDefaultExtension() const
{
    return "opli";
}
...
FfmtErrCode WohlstandOPL3_GM::saveFile(std::string filePath, FmBank &bank)
{
    FmBank gm_bank = bank;
...
std::string WohlstandOPL3_GM::formatName() const
{
    return "Standard OPL3 GM bank by Wohlstand";
}

std::string WohlstandOPL3_GM::formatModuleName() const
{
    return "WOPL GeneralMidi Writer";
}

std::string WohlstandOPL3_GM::formatExtensionMask() const
{
    return "*.wopl";
}

std::string WohlstandOPL3_GM::formatDefaultExtension() const
{
    return "wopl";
}
