...
video::IImage* SGUITTGlyph::createGlyphImage(const FT_Bitmap& bits, video::IVideoDriver* driver) const
{
	...
	switch (bits.pixel_mode)
	{
		...
		default:
            logging::Error("Load Font: Unsupported pixel mode, Error");
			return 0;
	}
	return image;
}
...
void SGUITTGlyph::preload(u32 char_index, FT_Face face, video::IVideoDriver* driver, u32 font_size, const FT_Int32 loadFlags)
{
	...
	{
        logging::Error("Preload Font: Font preloading failed");
		return;
	}
	...
	if (!page)
	{
		...
		if (!page)
		{
            logging::Error("Preload Font: createGlyphPage failed, Error");
			return;
		}
	}
...
void CGUITTGlyphPage::updateTexture()
{
	...
	for (u32 i = 0; i < glyph_to_be_paged.size(); ++i)
	{
		...
		if (glyph && glyph->isLoaded)
		{
			...
			{
                logging::Error("Font: updateTexture failed, Error");
			}
		}
	}
...
bool CGUITTFont::load(const io::path& filename, u32 size, bool antialias, bool transparency)
{
    ...
    //Note 02.10.2025: Modified original authors source code so that
    //logging is done via this projects logger implementation
    std::string logMsg("CGUITTFont: Creating new font: ");
    logMsg.append(filename.c_str());
    logMsg.append(" ");

    char hlpstr[500];
    snprintf(hlpstr, 500, "%u",size);

    logMsg.append(hlpstr);
    logMsg.append("pt ");

    if (antialias) {
        logMsg.append("+antialias");
    } else {
        logMsg.append("-antialias");
    }

    if (transparency) {
        logMsg.append("+transparency");
    } else {
        logMsg.append("-transparency");
    }
	...
	if (node == 0)
	{
		...
		if (FileSystem)
		{
			...
			if (file == 0)
			{
                ...
                logging::Error("CGUITTFont: Failed to open the file.");
			...
			if (FT_New_Memory_Face(c_library, face->face_buffer, face->face_buffer_size, 0, &face->face))
			{
                ...
                logging::Error("CGUITTFont: FT_New_Memory_Face failed.");
		...
		else
		{
			...
			if (FT_New_Face(c_library, converter.c_str(), 0, &face->face))
			{
                ...
                logging::Error("CGUITTFont: FT_New_Face failed.");
...
CGUITTGlyphPage* CGUITTFont::createGlyphPage(const u8& pixel_mode)
{
	...
	if (page)
	{
        if (!page->createPageTexture(pixel_mode, page_texture_size)) {
            logging::Error("CGUITTFont: createGlyphPage failed.");
			return 0;
        }
