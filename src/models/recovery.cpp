...
void Recovery::State_PlayerGrabbed(irr::f32 deltaTime) {
    ...
    if (mPlayerDropOfDirVecFound) {
        ...
        if (isnan(currPlayerAbsAngle) == 1) {
            //current player craft angle is NaN, most likely after pyhsics reset
            //operation of this player, workaround, just forget about the angle
            mPlayerDropOfDirVecFound = false;
